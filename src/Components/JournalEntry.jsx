...
const JournalEntry = () => {
    ...
    const fetchJournal = async () => {
      ...
      try {
        const response = await axios.get(
          ...
          }/journals/${anonymousName}/${slug}`
        );
        setEntry(response.data.journal);

        const wordCount =
          response.data.journal.content?.replace(/<[^>]*>/g, "").split(/\s+/)
            .length || 0;
  ...
  useEffect(() => {
    setRecommendations([]); // Clear recommendations when entry changes/loading
  }, [anonymousName, slug]);
  ...
  useEffect(() => {
    ...
    // Use GET to check subscription status
    axios.get(`${import.meta.env.VITE_API_URL}/subscription-status/${currentUser._id}/${author._id}`)
      .then(res => {
  ...
  useEffect(() => {
    if (entry && entry.userId && typeof entry.userId === "object") {
      axios.get(`${import.meta.env.VITE_API_URL}/user/${entry.userId._id}`)
  ...
  const handleLike = async () => {
    ...
    setLikeLoading(true);
    try {
      const res = await axios.post(`${import.meta.env.VITE_API_URL}/journals/${entry._id}/like`, {
      ...
      <div className="min-h-screen flex items-center justify-center bg-white dark:bg-gray-900">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-8 leading-relaxed">
            {error || contextError || "This journal entry doesn't exist."}
          </p>
          <button
            onClick={() => navigate(-1)}
