...
const RelatedTags = ({ currentTag, contentType, onTagSelect }) => {
  const storyTags = ['Fantasy', 'Horror', 'Science Fiction', 'Romance', 'Mystery', 'Adventure', 'Drama', 'Comedy'];
  const journalTags = ['Personal', 'Reflection', 'Life', 'Growth', 'Thoughts', 'Experience', 'Daily', 'Mindfulness'];
  
  const tags = contentType === 'stories' ? storyTags : journalTags;
  const relatedTags = tags.filter(tag => tag.toLowerCase() !== currentTag.toLowerCase()).slice(0, 6);
...
const TagHeroSection = ({ tag, totalCount, contentType }) => {
  const getTagDescription = (tagName) => {
    const descriptions = {
      // Story tags
      'Fantasy': 'Dive into magical realms filled with wonder, mythical creatures, and extraordinary adventures.',
      'Horror': 'Experience spine-chilling tales that will keep you on the edge of your seat.',
      'Science Fiction': 'Explore futuristic worlds and cutting-edge technology through imaginative storytelling.',
      'Romance': 'Discover heartwarming love stories that celebrate human connection and emotion.',
      'Mystery': 'Unravel puzzles and secrets in these captivating tales of intrigue.',
      'Adventure': 'Embark on thrilling journeys filled with excitement and discovery.',
      'Drama': 'Experience powerful stories that explore the depths of human emotion.',
      'Comedy': 'Enjoy lighthearted tales that bring joy and laughter.',
      
      // Journal tags
      'Personal': 'Intimate reflections and personal experiences shared authentically.',
      'Reflection': 'Thoughtful contemplations on life, growth, and self-discovery.',
    ...
    return descriptions[tagName] || `Explore ${contentType} tagged with "${tagName}" from our community of writers.`;
  };
...
const TagEntries = () => {
  ...
  const fetchEntries = useCallback(async (pageNum = 1, append = false) => {
    try {
      ...
      if (user) {
        const liked = new Set(newEntries.filter(entry => entry.likes?.includes(user._id)).map(entry => entry._id));
        const saved = new Set(newEntries.filter(entry => user.savedEntries?.includes(entry._id)).map(entry => entry._id));
  ...
  // Handle like
  const handleLike = useCallback(async (entry) => {
    if (!user) return;
  ...
  // Handle save
  const handleSave = useCallback(async (entryId) => {
    if (!user) return;
  ...
  // Dynamic SEO content
  const seo = {
    title: `${contentType === 'stories' ? 'Stories' : 'Journals'} Tagged "${tag}" | Starlit Journals`,
    description: `Discover ${contentType === 'stories' ? 'creative stories' : 'personal journal entries'} tagged with "${tag}". Find meaningful content from our community of writers.`,
    keywords: `${tag.toLowerCase()}, ${contentType}, creative writing, ${contentType === 'stories' ? 'short stories, fiction' : 'personal journal, daily writing'}, starlit journals`,
  ...
  return (
    <>
      ...
      <div className="min-h-screen">
        <div className='p-6'>
        <TagHeroSection tag={tag} totalCount={totalCount} contentType={contentType} />
        <RelatedTags currentTag={tag} contentType={contentType} onTagSelect={handleTagSelect} />
        </div>
        <div className="max-w-7xl mx-auto px-4 py-8">
          {entries.length === 0 ? (
            <div className="text-center py-12">
              <div className="max-w-md mx-auto">
                ...
                <p className="text-gray-600 mb-6">
                  ...
                  Try exploring other tags or check back later.
                </p>
                <button
